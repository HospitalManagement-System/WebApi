...
namespace ServiceLayer.Services.Email
{
    public class EmailSender : IEmailSender
    {
        ...
        public async Task<string> PatientSendLoginSMSAsync(string UserName, string Password, string Email)
        {

            var currentDirectory = System.IO.Directory.GetCurrentDirectory();
            var FilePath = $"{currentDirectory.Replace("\\LoginAPI", "")}\\ServiceLayer\\EmailTemplates\\EmployeeRegistartion.html";
            StreamReader str = new StreamReader(FilePath);
            string MailText = str.ReadToEnd();
            str.Close();
            //Replace Email
        ...
        public async Task<string> ForgotPassword(string Email,string UserName)
        {

            var currentDirectory = System.IO.Directory.GetCurrentDirectory();
            var FilePath = $"{currentDirectory.Replace("\\LoginAPI", "")}\\ServiceLayer\\EmailTemplates\\ForgotPassword.html";
            StreamReader str = new StreamReader(FilePath);
            string MailText = str.ReadToEnd();
            str.Close();


            MailText = MailText.Replace("[user]", UserName).Replace("[password]", "Password@123");
